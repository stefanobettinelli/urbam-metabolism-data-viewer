export const convertArrayOfObjectsToCSV = args => {
  let csvStringResult =
    'box_name; timestamp_cest; timestamp_utc; co; co2; no2; o3; pm2.5; pm10; voc; air_t_ds; air_t_am; rad_am\n';
...
export const getHeaderAsJSONFromCSVText = csvText => {
  ...
  const header = records[0]
    .split(';')
    .map(item => ({ Header: item, accessor: item.toLowerCase() }));
...
export const convertCSVBlobTextToListOfJSON = csvText => {
  ...
  return records.map(record => {
    let obj = {};
    record.split(';').forEach(function(item, index) {
